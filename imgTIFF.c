 ...
 *
 * Style control:
 *   indent -br -brs -ce -nut -cdw -lp -hnl imgTIFF.c
 */
...
static int ChnMatchTIFF
_ANSI_ARGS_ ((Tcl_Interp * interp, Tcl_Channel chan, char *fileName,
              Tcl_Obj * format, int *widthPtr, int *heightPtr));
static int ObjMatchTIFF
_ANSI_ARGS_ ((Tcl_Interp * interp, Tcl_Obj * dataObj, Tcl_Obj * format,
...
static int ObjMatchTIFF
_ANSI_ARGS_ ((Tcl_Obj * dataObj, Tcl_Obj * format, int *widthPtr,
              int *heightPtr, Tcl_Interp * interp));
#endif

static int ChnReadTIFF _ANSI_ARGS_ ((Tcl_Interp * interp, Tcl_Channel chan,
                                     char *fileName, Tcl_Obj * format,
                                     Tk_PhotoHandle imageHandle, int destX,
                                     int destY, int width, int height,
                                     int srcX, int srcY));
static int ObjReadTIFF
_ANSI_ARGS_ ((Tcl_Interp * interp, Tcl_Obj * dataObj, Tcl_Obj * format,
              Tk_PhotoHandle imageHandle, int destX, int destY, int width,
              int height, int srcX, int srcY));
static int ChnWriteTIFF
_ANSI_ARGS_ ((Tcl_Interp * interp, char *filename, Tcl_Obj * format,
              Tk_PhotoImageBlock * blockPtr));
static int StringWriteTIFF
_ANSI_ARGS_ ((Tcl_Interp * interp, Tcl_DString * dataPtr, Tcl_Obj * format,
              Tk_PhotoImageBlock * blockPtr));
...
static struct TiffFunctions {
  ...
  TIFF *(*Open) _ANSI_ARGS_ ((CONST char *, CONST char *));
    tsize_t (*ReadEncodedStrip)
    _ANSI_ARGS_ ((TIFF *, tstrip_t, tdata_t, tsize_t));
  int (*ReadRGBAImage) _ANSI_ARGS_ ((TIFF *, uint32, uint32, uint32 *, int));
    tsize_t (*ReadTile)
    _ANSI_ARGS_ ((TIFF *, tdata_t, uint32, uint32, uint32, tsample_t));
  int (*SetField) _ANSI_ARGS_ ((TIFF *, TCL_VARARGS2 (ttag_t, tag)));
    tsize_t (*TileSize) _ANSI_ARGS_ ((TIFF *));
    tsize_t (*WriteEncodedStrip)
    _ANSI_ARGS_ ((TIFF *, tstrip_t, tdata_t, tsize_t));
  ...
  TIFFCodec *(*RegisterCODEC)
    _ANSI_ARGS_ ((uint16, CONST char *, TIFFInitMethod));
  void (*Error)
    _ANSI_ARGS_ ((CONST char *, TCL_VARARGS2 (CONST char *, arg1)));
  ...
  int (*ReadScanline) _ANSI_ARGS_ ((TIFF *, tdata_t, uint32));
  ...
  0, TIFFClose,
    ...
    TIFFReadRGBAImage,
    TIFFReadTile, TIFFSetField, TIFFTileSize, TIFFWriteEncodedStrip,
    ...
    _TIFFrealloc,
    TIFFSetErrorHandler,
    TIFFSetWarningHandler, TIFFClientOpen, TIFFRegisterCODEC, TIFFError,
    /* TIFFPredictorInit, */ 0,
    /* _TIFFMergeFieldInfo, */ 0,
    /* TIFFFlushData1, */ 0,
    /* _TIFFNoPostDecode, */ 0,
    TIFFTileRowSize, TIFFScanlineSize, TIFFReadScanline,
    /* _TIFFsetByteArray, */ 0,
...
static int CommonReadTIFF _ANSI_ARGS_ ((Tcl_Interp * interp, TIFF * tif,
                                        ...
                                        int destY, int width, int height,
                                        int srcX, int srcY));
static int CommonWriteTIFF
_ANSI_ARGS_ ((Tcl_Interp * interp, TIFF * tif, int comp,
              Tk_PhotoImageBlock * blockPtr));
static int ParseWriteFormat
_ANSI_ARGS_ ((Tcl_Interp * interp, Tcl_Obj * format, int *comp, char **mode));
...
void ImgTIFFMergeFieldInfo
_ANSI_ARGS_ ((TIFF * tif, CONST VOID * voidp, int i));
...
int ImgTIFFReadScanline _ANSI_ARGS_ ((TIFF *, tdata_t, uint32));
...
int
ImgTIFFReadScanline (tif, scanline, row)
     TIFF *tif;
     tdata_t scanline;
     uint32 row;
{
  return tiff.ReadScanline (tif, scanline, row);
}
     ...
     int order;
{
  ...
  case TIFF_BYTE:
    ...
    break;
  case TIFF_SHORT:
    result = (buf[order] << 8) + buf[1 - order];
    break;
  case TIFF_LONG:
    if (order) {
...
#ifdef USE_IMGSEEK
  return (fsize =
          ImgSeek ((MFile *) fd, 0, SEEK_END)) < 0 ? 0 : (toff_t) fsize;
#else
...
#else
static int
ObjMatchTIFF (data, format, widthPtr, heightPtr, interp)
#endif
     Tcl_Interp *interp;
...
#if TK_MAJOR_VERSION == 8 && TK_MINOR_VERSION == 0
static int
ChnMatchTIFF (interp, chan, fileName, format, widthPtr, heightPtr)
#else
static int
ChnMatchTIFF (chan, fileName, format, widthPtr, heightPtr, interp)
#endif
     Tcl_Interp *interp;
     Tcl_Channel chan;
     char *fileName;
...
static int
CommonMatchTIFF (handle, widthPtr, heightPtr)
     MFile *handle;
     int *widthPtr, *heightPtr;
{
...
static int
ObjReadTIFF (interp, data, format, imageHandle,
     ...
     int srcX, srcY;
{
  ...
  if (tiff.ClientOpen) {
    ...
    if (Tcl_SetChannelOption (interp, outchan, "-translation", "binary") !=
        TCL_OK) {
      return TCL_ERROR;
    }
...
static int
ChnReadTIFF (interp, chan, fileName, format, imageHandle,
             destX, destY, width, height, srcX, srcY)
     Tcl_Interp *interp;
     Tcl_Channel chan;
     char *fileName;
...
static int
CommonReadTIFF (interp, tif, format, imageHandle,
                destX, destY, width, height, srcX, srcY)
     Tcl_Interp *interp;
     TIFF *tif;
     Tcl_Obj *format;
     ...
     int srcX, srcY;
{
  ...
  if (datatype == SAMPLEFORMAT_IEEEFP) {
    if (bps != 32) {
      ...
      Tcl_AppendResult (interp, "Only 32-bit Float-TIFFs supported",
                        (char *) NULL);
      return TCL_ERROR;
    }
    uint32 row;
    ...
    for (row = 0; row < h; row++) {
      ...
      tiff.ReadScanline (tif, fscanline, row);
      for (col = 0; col < w; col++) {
    ...
    if (hist_equal) {
      ...
      for (i = 0; i < npixels; i++) {
        uint8 val = (uint8) (scale_max * (fraster[i] + min));
        hist[val]++;
      }
      hist[0] = 0;
...
#ifdef WORDS_BIGENDIAN
      uint8 *a = (uint8 *) praster;
      uint8 *b = (uint8 *) praster + 1;
      uint8 *g = (uint8 *) praster + 2;
      uint8 *r = (uint8 *) praster + 3;
#else
      uint8 *r = (uint8 *) praster;
      uint8 *g = (uint8 *) praster + 1;
      uint8 *b = (uint8 *) praster + 2;
      uint8 *a = (uint8 *) praster + 3;
#endif
      *r = *g = *b = val;
      *a = 0;
    }
...
static int
StringWriteTIFF (interp, dataPtr, format, blockPtr)
     Tcl_Interp *interp;
     Tcl_DString *dataPtr;
     Tcl_Obj *format;
     Tk_PhotoImageBlock *blockPtr;
{
  ...
  if (tempFileName[0]) {
    ...
    if (Tcl_SetChannelOption (interp, inchan, "-translation", "binary") !=
        TCL_OK) {
      return TCL_ERROR;
    }
...
static int
ChnWriteTIFF (interp, filename, format, blockPtr)
     Tcl_Interp *interp;
     char *filename;
     Tcl_Obj *format;
     Tk_PhotoImageBlock *blockPtr;
{
  ...
  if ((fullname =
       Tcl_TranslateFileName (interp, filename, &nameBuffer)) == NULL) {
    return TCL_ERROR;
  }
  ...
  if (objc) {
    ...
    for (i = 1; i < objc; i++) {
      ...
      case 0:
        ...
        break;
      case 1:
        byteorder = Tcl_GetStringFromObj (objv[i], (int *) NULL);
        break;
      }
    }
    c = compression[0];
    length = strlen (compression);
    if ((c == 'n') && (!strncmp (compression, "none", length))) {
      ...
      *comp = COMPRESSION_JPEG;
               ...
               && (!strncmp (compression, "logluv", length))) {
      *comp = COMPRESSION_SGILOG;
#endif
    } else if ((c == 'l') && (length > 1)
               && (!strncmp (compression, "lzw", length))) {
      *comp = COMPRESSION_LZW;
    } else if ((c == 'p') && (length > 1)
               && (!strncmp (compression, "packbits", length))) {
      *comp = COMPRESSION_PACKBITS;
    } else if ((c == 'p') && (length > 1)
               && (!strncmp (compression, "pixarlog", length))) {
      *comp = COMPRESSION_PIXARLOG;
    } else {
      Tcl_AppendResult (interp, "invalid compression mode \"",
                        compression,
                        "\": should be deflate, jpeg, logluv, lzw, ",
...
static int
CommonWriteTIFF (interp, tif, comp, blockPtr)
     Tcl_Interp *interp;
     TIFF *tif;
     int comp;
     Tk_PhotoImageBlock *blockPtr;
